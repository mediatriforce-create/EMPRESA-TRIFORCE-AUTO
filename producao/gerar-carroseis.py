...
.swipe { font-size:22px; font-weight:600; color:#FF6B00; }
"""

_LOGO_PATH = Path(__file__).parent / "banco-de-imagens" / "LOGOS TRIFORCE AUTO" / "HORIZONTAL SO O TEXTO SEM FUNDO E TEXTO PRETO.png"
_logo_b64 = ""
...
SLIDE_FOOTER_HTML = f"""
  <div class="footer">
    <div class="footer-left">
      {'<div style="position:relative;width:320px;height:42px;overflow:visible;flex-shrink:0;"><img src="' + _logo_b64 + '" style="position:absolute;bottom:-80px;left:0;height:190px;width:auto;object-fit:contain;" alt="Triforce Auto"></div>' if _logo_b64 else '<div class="footer-avatar"><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" fill="white"/></svg></div><div><div class="footer-name">Triforce Auto</div><div class="footer-handle">@triforceauto</div></div>'}
    </div>
...
def _img_box(logo_b64=None, bg_file=None, logo_domain=None, height="flex", fit="cover", bg="#0A0A0A", padding=None):
    ...
    padding — ex: '0 24px' adiciona margem lateral + border-radius ao container externo
    """
    h_style = "flex:1; min-height:240px;" if height == "flex" else f"height:{height}; flex-shrink:0;"

    if logo_b64:
        inner = f"""<div style="width:100%; height:100%; border-radius:12px; overflow:hidden;
    background:#111; display:flex; align-items:center; justify-content:center;">
    <img src="{logo_b64}" style="max-height:200px;max-width:620px;object-fit:contain;">
  </div>"""
        if padding:
            return f'<div style="{h_style} padding:{padding}; box-sizing:border-box;">{inner}</div>'
        return f'<div style="width:100%; {h_style} border-radius:20px; overflow:hidden; background:#111; display:flex; align-items:center; justify-content:center;"><img src="{logo_b64}" style="max-height:200px;max-width:620px;object-fit:contain;"></div>'
    ...
    <img src="data:{mime};base64,{b64}"
         style="max-width:100%; max-height:100%; object-fit:contain; border-radius:12px; display:block;">
            ...
            if padding:
                return f'<div style="{h_style} padding:{padding}; box-sizing:border-box;">{inner}</div>'
            return f'<div style="width:100%; {h_style} border-radius:20px; overflow:hidden; background:{bg}; display:flex; align-items:center; justify-content:center; padding:8px;"><img src="data:{mime};base64,{b64}" style="max-width:100%; max-height:100%; object-fit:contain; border-radius:12px; display:block;"></div>'
        else:
            if padding:
                inner = f'<div style="width:100%; height:100%; border-radius:12px; overflow:hidden; background:url(\'data:{mime};base64,{b64}\') center/cover no-repeat;"></div>'
                return f'<div style="{h_style} padding:{padding}; box-sizing:border-box; margin-bottom:0;">{inner}</div>'
    ...
    fallback_inner = f"""<div style="width:100%; height:100%; border-radius:12px;
    background:#0A0A0A; display:flex; align-items:center; justify-content:center;">{inner}</div>"""
    if padding:
        return f'<div style="{h_style} padding:{padding}; box-sizing:border-box;">{fallback_inner}</div>'
...
def content_slide(num, title, body_html, logo_domain=None, logo_b64=None, bg_file=None,
                  layout="standard", chart=None, accent=None, photo_height=None, photo_fit="cover", photo_bg="#0A0A0A", photo_padding=None):
    """
    ...
    photo_padding         — ex: "0 24px" adiciona margem lateral + border-radius à foto
    """
    ...
    if layout in ("standard", "heavy"):
        ...
        img = _img_box(logo_b64, bg_file, logo_domain, height=h, fit=photo_fit, bg=photo_bg, padding=photo_padding)
        if layout == "heavy":
...
CARROSEIS = [
    ...
    # ── CARROSSEL 14 — LP que não converte (copy Mateus) ──────────────────────
    {
        "slug": "14-lp-nao-converte",
        "cover_photo_query": "dark minimal typography bold text black background editorial",
        "slides": [
            ("cover", {
                "headline": "5 erros que afastam cliente antes de ele clicar.",
                "subheadline": "Se a sua landing page não converte, o problema provavelmente está aqui",
            }),
            ("content", {
                "num": 2,
                "layout": "standard",
                "photo_query": "person typing laptop computer writing focused workspace",
                "photo_height": "flex",
                "title": "Seu título não diz nada.",
                "body": "&ldquo;Bem-vindo ao meu salão&rdquo; não convence ninguém.<br><br>O cliente chegou com uma dor. Seu título precisa bater nela.<br><br><strong>Título vago = cliente fecha e vai pro próximo.</strong>",
            }),
            ("content", {
                "num": 3,
                "layout": "standard",
                "photo_query": "happy customer holding smartphone smiling",
                "photo_height": "flex",
                "photo_fit": "contain",
                "photo_padding": "0 24px",
                "title": "Ninguém acredita em você ainda.",
                "body": "Depoimento real de cliente vale mais que qualquer texto que você escrever sobre si mesmo.<br><br>Pet shop sem avaliação visível perde pro concorrente com 12 fotos de clientes satisfeitos.<br><br><strong>Prova social não é opcional.</strong>",
            }),
            ("content", {
                "num": 4,
                "layout": "standard",
                "photo_query": "person clicking call to action button smartphone green whatsapp screen",
                "photo_height": "flex",
                "title": "&ldquo;Saiba mais&rdquo; não agenda ninguém.",
                "body": "Botão genérico = cliente não sabe o que vai acontecer ao clicar.<br><br>Troca por algo direto: <strong>&ldquo;Agendar agora no WhatsApp&rdquo;</strong>.<br><br>Um clique tem que ter uma consequência clara.",
            }),
            ("content", {
                "num": 5,
                "layout": "heavy",
                "photo_query": "hand holding smartphone broken website layout mobile screen",
                "photo_height": "flex",
                "title": "No celular, sua LP some.",
                "body": "<strong>83 em cada 100 visitantes</strong> chegam pelo celular.<br><br>Se o botão não abre direito ou o texto corta, o cliente some.<br><br>1 segundo a mais de carregamento já reduz conversão.",
            }),
            ("content", {
                "num": 6,
                "layout": "heavy",
                "photo_query": "overwhelmed person too many choices decision stress",
                "photo_height": "flex",
                "title": "Uma página, um objetivo.",
                "body": "LP com menu, Instagram, formulário e WhatsApp ao mesmo tempo confunde.<br><br>Cliente com muita escolha não escolhe nada. Sai.<br><br><strong>Cada campo a mais que você coloca tira cliente da rota.</strong>",
            }),
            ("content", {
                "num": 7,
                "layout": "spotlight",
                "accent": "Copy + design\n+ sistema.\nTudo junto.",
                "title": "Copy + design + sistema. Tudo junto.",
                "body": "Não adianta ter design bonito com texto que não converte. Nem texto bom em página que quebra no celular. Quando os três trabalham juntos, a página faz o trabalho. Você só atende.",
            }),
            ("cta", {
                "main": "Sua LP no ar do <span>jeito certo</span>.",
                "sub": "A Triforce Auto entrega copy, design e sistema no mesmo pacote. Sem enrolação, sem depender de agência cara. Chama no WhatsApp e a gente começa.",
            }),
        ]
    },

    # ── CARROSSEL — Apresentação do Perfil ─────────────────────────────────────
    {
        "slug": "apresentacao-perfil",
        "cover_photo_query": "person standing alone dark dramatic cinematic light laptop screen glow shadow",
        "slides": [
            ("cover", {
                "headline": "Você não vai ser substituído por uma IA.",
                "subheadline": "Vai ser substituído por alguém que usa. E esse alguém já começou.",
            }),
            ("content", {
                "num": 2,
                "layout": "standard",
                "photo_query": "brazil city work technology",
                "photo_height": "flex",
                "title": "O Brasil não está atrasado. Está na frente.",
                "body": "26% dos brasileiros já usam IA todo dia. Quase o dobro da média mundial. A adoção aconteceu. <strong>Você está dentro ou fora?</strong>",
            }),
            ("content", {
                "num": 3,
                "layout": "standard",
                "photo_query": "small shop owner using smartphone at store counter serving customer",
                "photo_height": "flex",
                "title": "IA não é robô de ficção científica.",
                "body": "É a ferramenta que responde e-mail, organiza estoque e atende cliente enquanto você faz outra coisa. Quem usa trabalha mais em menos tempo. <strong>Quem não usa, trabalha mais.</strong>",
            }),
            ("content", {
                "num": 4,
                "layout": "standard",
                "photo_query": "focused person working laptop open spreadsheet data home office natural light",
                "photo_height": "flex",
                "title": "Produtividade sobe até 40% com IA em pequenas empresas.",
                "body": "Não é magia. É processo. Quem configurou certo faz em 2 horas o que antes levava 5. <strong>A diferença não é o tamanho da empresa. É a ferramenta na mão.</strong>",
            }),
            ("content", {
                "num": 5,
                "layout": "standard",
                "photo_query": "small shop owner smiling counter",
                "photo_height": "flex",
                "title": "Uma loja com 4 funcionários integrou IA.",
                "body": "Erros de estoque caíram 75%. Custo operacional caiu 18%. Não foi uma multinacional. Foi um negócio igual ao seu. <strong>44% das pequenas empresas brasileiras já chegaram lá.</strong>",
            }),
            ("content", {
                "num": 6,
                "layout": "spotlight",
                "accent": "A IA não decide\no futuro do\nseu negócio.",
                "title": "A IA não decide o futuro do seu negócio.",
                "body": "Você decide. Mas só quem entende a ferramenta tem essa escolha.",
            }),
            ("cta", {
                "main": "Aqui a gente fala de IA do jeito que <span>funciona na prática</span>.",
                "sub": "Sem teoria. Sem hype. Com caso real e aplicação direta. Segue o perfil e fica por dentro toda semana. @triforceauto",
            }),
        ]
    },
...
async def main():
    ...
    carroseis_to_run = [c for c in CARROSEIS if c["slug"] == slug_filter] if slug_filter else CARROSEIS
    if slug_filter and not carroseis_to_run:
        print(f"[erro] Slug '{slug_filter}' nao encontrado em CARROSEIS.")
        return

    print("\nTriforce Auto - Gerador de Carrosseis")
    if slug_filter:
        print(f"  Filtrando: {slug_filter}")
    ...
    async with async_playwright() as p:
        ...
        slide_photos = await download_slide_photos(browser, carroseis_to_run)
        screenshots  = await capture_screenshots(browser, carroseis_to_run)

        # Etapa 2: gerar slides
        print("\nGerando slides...")
        page = await browser.new_page(viewport={"width": 1080, "height": 1350})

        for carousel in carroseis_to_run:
            slug = carousel["slug"]
            ...
            for i, (slide_type, data) in enumerate(carousel["slides"]):
                ...
                if slide_type == "cover":
                    ...
                    html = content_slide(
                        ...
                        photo_padding=data.get("photo_padding"),
                    )
    ...
    total = sum(len(c["slides"]) for c in carroseis_to_run)
    print(f"\nPronto! {total} slides em {len(carroseis_to_run)} carrosseis")
    print(f"   {OUTPUT_DIR}")
